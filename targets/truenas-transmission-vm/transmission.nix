...
in {
  ...
  config = mkIf cfg.enable {
    ...
    system.activationScripts.transmission-daemon = ''
      ...
      install -d -m '${cfg.downloadDirPermissions}' '${cfg.settings.download-dir}'
    '' + optionalString cfg.settings.incomplete-dir-enabled ''
      install -d -m '${cfg.downloadDirPermissions}' '${cfg.settings.incomplete-dir}'
    '';

    # This is the 20.09 release module for Transmission. I removed the -o and -g options in the lines above to prevent impossible chowns/chmods on NFS directories:
    ...
    assertions = [
      ...
      }
      {
        ...
        message =
          "`services.transmission.settings.download-dir' must be an absolute path.";
      }
      {
        assertion = types.path.check cfg.settings.incomplete-dir;
        message =
          "`services.transmission.settings.incomplete-dir' must be an absolute path.";
      }
      {
        assertion = types.path.check cfg.settings.watch-dir;
        message =
          "`services.transmission.settings.watch-dir' must be an absolute path.";
      }
      {
        assertion = cfg.settings.script-torrent-done-filename == ""
          || types.path.check cfg.settings.script-torrent-done-filename;
        message =
          "`services.transmission.settings.script-torrent-done-filename' must be an absolute path.";
      }
      {
        assertion = types.port.check cfg.settings.rpc-port;
        message = "${
            toString cfg.settings.rpc-port
          } is not a valid port number for `services.transmission.settings.rpc-port`.";
      }
      # In case both port and settings.rpc-port are explicitely defined: they must be the same.
      {
        assertion = !options.services.transmission.port.isDefined || cfg.port
          == cfg.settings.rpc-port;
        message =
          "`services.transmission.port' is not equal to `services.transmission.settings.rpc-port'";
      }
    ];

    services.transmission.settings =
      optionalAttrs options.services.transmission.port.isDefined {
        rpc-port = cfg.port;
      };
